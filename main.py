...
random_index = random.randint(0, len(test_images) - 1)  # Выбираем случайный индекс
plt.imshow(test_images[random_index], cmap="gray")
plt.title(
    f"Predicted: {class_names[np.argmax(predictions[random_index])]}, Actual: {class_names[test_labels[random_index]]}"
)
plt.show()
